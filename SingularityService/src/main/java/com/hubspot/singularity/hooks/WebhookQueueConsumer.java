...
public class WebhookQueueConsumer implements QueueConsumer<WebhookQueuedJob> {
  ...
  public WebhookQueueConsumer(ObjectMapper objectMapper) {
    ...
    builder.setConnectionTimeoutInMs(30 * 1000); // default is 60 * 1000
  ...
  public void consumeMessage(WebhookQueuedJob job) throws Exception {
    ...
    try {
      ...
      // because on failure we should throw an exception to force message to remain in queue and be re-consumed
      Response response = asyncHttpClient.preparePost(job.getHookURI()).setBody(job.getTaskUpdate().getAsBytes(objectMapper)).addHeader("Content-Type", "application/json").execute(handler).get();
